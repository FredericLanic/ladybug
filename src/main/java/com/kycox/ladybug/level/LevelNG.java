...
public abstract class LevelNG implements ILevel {
	...
	protected int screenBlockValue(int x, int y) {
		int pointValue = 0;
		...
		else pointValue += RIGHT;
		

		if (block == N) {
			pointValue += NOT_ACCESSIBLE_POINT;
			
			if(blockUp != null && blockUp != N) pointValue += UP;
			if(blockDown != null && blockDown != N) pointValue += DOWN;
			if(blockLeft != null && blockLeft != N) pointValue += LEFT;
			if(blockRight != null && blockRight != N) pointValue += RIGHT;
