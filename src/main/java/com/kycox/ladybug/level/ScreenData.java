...
public final class ScreenData {
  ...
  public ScreenBlock getDataBlock(Point posPoint) {
    // java8 :
    return lstDataBlocks.stream().filter(b -> b.getCoordinate().equals(posPoint)).findFirst()
        .orElse(null);
  }
  ...
  /**
   * Retourne la map afficher dans le level
   *
   * @return
   */
  public List<ScreenBlock> getLstViewBlocks() {
    return lstViewBlocks;
  }
  ...
  public ScreenBlock getViewBlock(Point posPoint) {
    // java8 :
    return lstViewBlocks.stream().filter(b -> b.getCoordinate().equals(posPoint)).findFirst()
        .orElse(null);
  }
  ...
  public void updateScreenBlock(LadybugActions ladybugActions) {
    if (ladybugActions.hasEatenAMegaPoint() || ladybugActions.hasEatenAPoint()) {
      ...
      lstViewBlocks.stream()
          .filter(sb -> sb.getCoordinate().equals(currentScreenBlock.getCoordinate())).findFirst()
          .orElse(null).removePoint();
    }
  }
